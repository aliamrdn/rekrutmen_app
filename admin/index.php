   ...
   <?php include "menu.php"; ?>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <!-- End of Topbar -->
        <?php
          $cek_k = mysqli_query($koneksi, "SELECT COUNT(id_c_pekerja) FROM pekerja where ojt_status='0'");
          $row_k = mysqli_fetch_array($cek_k);
          $data_k = ceil($row_k[0]);
          
          $cek_laki = mysqli_query($koneksi, "SELECT COUNT(jenis_kelamin) FROM pekerja WHERE jenis_kelamin = 'L' AND ojt_status='0'");
          $row_l = mysqli_fetch_array($cek_laki);
          $data_laki = ceil($row_l[0]);

          $cek_p = mysqli_query($koneksi, "SELECT COUNT(jenis_kelamin) FROM pekerja WHERE jenis_kelamin = 'P' AND ojt_status='0'");
          $row_p = mysqli_fetch_array($cek_p);
          $data_p = ceil($row_p[0]);
          
          $cek_sma = mysqli_query($koneksi, "SELECT COUNT(pendidikan) FROM pekerja WHERE pendidikan = 'SMA' AND ojt_status='0'");
          $row_sma = mysqli_fetch_array($cek_sma);
          $data_sma = ceil($row_sma[0]);

          $cek_smk = mysqli_query($koneksi, "SELECT COUNT(pendidikan) FROM pekerja WHERE pendidikan = 'SMK' AND ojt_status='0'");
          $row_smk = mysqli_fetch_array($cek_smk);
          $data_smk = ceil($row_smk[0]);

          $cek_d3 = mysqli_query($koneksi, "SELECT COUNT(pendidikan) FROM pekerja WHERE pendidikan = 'D3' AND ojt_status='0'");
          $row_d3 = mysqli_fetch_array($cek_d3);
          $data_d3 = ceil($row_d3[0]);

          $cek_s1 = mysqli_query($koneksi, "SELECT COUNT(pendidikan) FROM pekerja WHERE pendidikan = 'S1' AND ojt_status='0'");
          $row_s1 = mysqli_fetch_array($cek_s1);
          $data_s1 = ceil($row_s1[0]);

          //ukuran seragam

          $cek_s = mysqli_query($koneksi, "SELECT COUNT(ukr_seragam) FROM pekerja WHERE ukr_seragam = 'S' AND ojt_status='0'");
          $row_s = mysqli_fetch_array($cek_s);
          $data_s = ceil($row_s[0]);

          $cek_m = mysqli_query($koneksi, "SELECT COUNT(ukr_seragam) FROM pekerja WHERE ukr_seragam = 'M' AND ojt_status='0'");
          $row_m = mysqli_fetch_array($cek_m);
          $data_m = ceil($row_m[0]);

          $cek_l = mysqli_query($koneksi, "SELECT COUNT(ukr_seragam) FROM pekerja WHERE ukr_seragam = 'L' AND ojt_status='0'");
          $row_lu = mysqli_fetch_array($cek_l);
          $data_l = ceil($row_lu[0]);

          $cek_xl = mysqli_query($koneksi, "SELECT COUNT(ukr_seragam) FROM pekerja WHERE ukr_seragam = 'XL' AND ojt_status='0'");
        ...
        <div class="container-fluid">
         <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Report Stok Pekerja</h1>
