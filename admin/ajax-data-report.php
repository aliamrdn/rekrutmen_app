...
if( !empty($requestData['search']['value']) ) {
	...
	$query=mysqli_query($conn, $sql) or die("ajax-data-report.php: get Part No"); // again run query with limit
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["ojt_status"] == '0'){
		$nestedData[] = '<td><center>STOK</a>
				     
	                 </center></td>';		
	} else{
		$nestedData[] = '<td><center>OJT
		</center></td>';	
	}
